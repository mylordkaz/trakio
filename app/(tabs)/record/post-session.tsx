...
function getBestLap(sessionDetail: SessionDetail | null) {
  ...
  return validTimedLaps.reduce((best, lap) => {
    if (!best || (lap.lapTimeMs ?? Number.MAX_SAFE_INTEGER) < (best.lapTimeMs ?? Number.MAX_SAFE_INTEGER)) {
      return lap;
    }
...
export default function PostSessionScreen() {
  ...
  const bestLapMs = bestLap?.lapTimeMs ?? sessionDetail?.session.bestLapMs ?? null;
  const sectorCount = getSectorCount(sessionDetail);
      ...
      >
        ...
        >
          <View className="flex-row items-center justify-between mb-4">
            ...
            <Text className="text-xs text-zinc-500 dark:text-zinc-400">
              {formatDateTime(sessionDetail?.session.endedAt ?? sessionDetail?.session.startedAt ?? null)}
            </Text>
          </View>
          ...
          <View className="flex-row items-start justify-between mb-5">
            <View className="flex-1 mr-3">
              ...
              <Text className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-white">
                {sessionDetail?.session.name ?? i18n.t('sessions.sessionNotFound')}
              </Text>
            </View>
          ...
          <View className="rounded-3xl bg-white/80 dark:bg-black/40 border border-zinc-200 dark:border-white/10 p-4">
            ...
            <View className="flex-row gap-2">
              {Array.from({ length: sectorCount }, (_, index) => {
                const sectorTime = bestLap?.sectors.find((sector) => sector.sectorIndex === index)?.splitTimeMs ?? null;

                return (
                  <View key={`best-sector-${index + 1}`} className="flex-1 rounded-2xl p-3 border bg-emerald-500/10 border-emerald-400/30">
                    <Text className="text-xs text-zinc-500 dark:text-zinc-400 mb-1">{`S${index + 1}`}</Text>
                    <Text className="text-lg font-medium text-zinc-900 dark:text-white">{formatSectorTime(sectorTime)}</Text>
                  </View>
                );
              })}
            </View>
        ...
        <View className="px-5 py-4 gap-4">
          <View className="flex-row gap-3">
            {[
              ...
              <View key={label} className="flex-1 rounded-2xl bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/10 p-3">
                <Text className="text-xs text-zinc-500 dark:text-zinc-400 mb-1">{label}</Text>
                <Text className="text-lg font-semibold text-zinc-900 dark:text-white">{value}</Text>
              </View>
            ))}
          </View>
          ...
          <Card>
            ...
            <View className="mb-4">
              <ProgressBar label={i18n.t('postSession.consistency')} value={`${consistency}%`} color="bg-white dark:bg-white" />
            </View>
              ...
              <View className="flex-row items-center gap-2">
                <View className="flex-1 h-2 rounded-full bg-zinc-200 dark:bg-white/10 overflow-hidden">
                  <View
                    ...
                    style={{
                      width: bestLapMs !== null && theoreticalBestMs !== null && bestLapMs > 0
                ...
                <Text className="text-xs text-zinc-500 dark:text-zinc-400">
                  {theoreticalBestMs !== null && bestLapMs !== null
                    ? i18n.t('sessions.gap', {
                        gap: ((bestLapMs - theoreticalBestMs) / 1000).toFixed(3),
                      })
                    : i18n.t('common.tbd')}
                </Text>
              </View>
                    ...
                    >
                      <View className="flex-row items-center gap-2">
                        <Text className="text-sm font-medium text-zinc-900 dark:text-white">
                          {i18n.t('sessions.lapLabel', { number: lap.lapNumber })}
                        </Text>
                        {isBest ? (
