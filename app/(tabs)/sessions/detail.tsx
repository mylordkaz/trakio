      ...
      >
        ...
        >
          ...
          <View className="flex-row items-start justify-between mb-5">
            <View className="flex-1 mr-3">
              <Text className="text-sm text-zinc-400">Recorded Session</Text>
              <Text className="text-2xl font-semibold tracking-tight text-white">Track Day · Session 2</Text>
              <Text className="text-sm text-zinc-400 mt-1">Mar 10, 2026 · 10:24 AM</Text>
            </View>
          ...
          <View className="rounded-3xl bg-black/40 border border-white/10 p-4">
            <View className="flex-row items-center justify-between mb-3">
              <Text className="text-sm text-zinc-400">Circuit View</Text>
              <Text className="text-sm text-zinc-400">Record Line</Text>
            </View>
            <View className="rounded-3xl border border-white/10 bg-zinc-950/80 p-4 mb-3 h-40 items-center justify-center">
              <Text className="text-zinc-500 text-sm">Track Map with Racing Line</Text>
            </View>
            <View className="flex-row gap-2">
              {SECTORS.map((s) => (
                <View key={s.label} className="flex-1 rounded-2xl p-3 border bg-violet-500/10 border-violet-400/20">
                  ...
                  <Text className="text-lg font-medium text-white">{s.value}</Text>
                </View>
              ))}
            </View>
            <View className="flex-row items-center justify-between mt-3">
              <Text className="text-xs text-zinc-400">Green = start / finish</Text>
              <Text className="text-xs text-zinc-400">Purple = fastest line</Text>
            </View>
          </View>
        </LinearGradient>
        ...
        <View className="px-5 py-4 gap-4">
          ...
          <View className="flex-row gap-3">
            {METRICS.map((m) => (
              <View key={m.label} className="flex-1 rounded-2xl bg-white/5 border border-white/10 p-3">
                <Text className="text-xs text-zinc-400 mb-1">{m.label}</Text>
                <Text className="text-lg font-semibold text-white">{m.value}</Text>
              </View>
            ))}
          </View>
          ...
          <Card>
            <View className="flex-row items-center justify-between mb-3">
              <View>
                <Text className="text-sm font-medium text-white">Session Insights</Text>
                <Text className="text-xs text-zinc-400">Quick performance summary</Text>
              </View>
              <Text className="text-sm text-violet-300">+1.2s vs last</Text>
            </View>
        ...
        <View className="px-5 pb-5 pt-1 flex-row gap-3">
          <Pressable className="flex-1 rounded-2xl border border-white/10 bg-white/5 py-3.5 items-center">
            <Text className="text-sm font-medium text-white">Export Data</Text>
          </Pressable>
